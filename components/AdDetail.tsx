...
export const renderAdDetail = (state, actions, adId) => {
  ...
  const images = ad.watermarkedImages || ad.images;
            ...
            <div class="carousel-inner rounded-3 shadow-sm">
              ${images.map((img, index) => `
                <div class="carousel-item ${index === 0 ? 'active' : ''}">
                  <img src="${img}" class="d-block w-100" alt="${i18n.tContent(ad.title)}" style="aspect-ratio: 1/1; object-fit: cover;" onerror="this.onerror=null;this.src='${PLACEHOLDER_IMG}';">
                </div>
              `).join('')}
            </div>
            ${images.length > 1 ? `
          ...
          <div class="mt-4 border-top pt-4">
            ...
            ${store ? `
              <div class="d-flex align-items-center gap-3 bg-light p-3 rounded-3">
                <img src="${store.logo}" alt="${store.name} logo" class="rounded-circle" style="width: 64px; height: 64px; object-fit: cover;" onerror="this.onerror=null;this.src='${PLACEHOLDER_IMG}';"/>
                <div>
